...
class Citizen:
    ...
    def remove_subordinate(self, cid: int) -> None:
        ...
        True
        """
        subordinates = self.get_direct_subordinates()
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        for subordinate in self._subordinates:
            citizen = subordinate.get_citizen(cid)
            if citizen:
                return citizen
        return None
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        for subordinate in self._subordinates:
            output = merge(output, subordinate.get_all_subordinates())
        return output
        # Note: This method must call itself recursively
    ...
    def get_society_head(self) -> Citizen:
        ...
        if not self._superior:
            ...
            output = self._superior.get_society_head()
            return output
        # Note: This method must call itself recursively
    ...
    def get_closest_common_superior(self, cid: int) -> Citizen:
        ...
        if self.get_citizen(cid) is not None:
            ...
            output = self._superior.get_closest_common_superior(cid)
            return output
        # Note: This method must call itself recursively
    ...
    def get_highest_rated_subordinate(self) -> Citizen:
        ...
        'Starky Industries'
        """
        max_sub = self._subordinates[0]
        for subordinate in self._subordinates:
            if subordinate.rating > max_sub.rating:
                max_sub = subordinate
        return max_sub
...
class Society:
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        if not superior_id:
            ...
            sup = self._head.get_citizen(superior_id)
    ...
    def change_citizen_type(self, cid: int,
        ...
        if not district_name:
            new_citizen = Citizen(cid, c.manufacturer, c.model_year,
                                  c.job, c.rating)
        else:
            new_citizen = DistrictLeader(cid, c.manufacturer, c.model_year,
                                         c.job, c.rating, district_name)
        if cid != self._head.cid:
            superior = c.get_superior()
            superior.remove_subordinate(cid)
            new_citizen.become_subordinate_to(superior)
        else:
            self._head = new_citizen
        for subordinate in c.get_direct_subordinates():
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        if isinstance(citizen.get_superior(), DistrictLeader):
            ...
            sup_superior = superior.get_superior()
            new_citizen = DistrictLeader(citizen.cid, citizen.manufacturer,
                                         citizen.model_year, superior.job,
                                         citizen.rating,
                                         superior.get_district_name())
            new_sup = Citizen(superior.cid, superior.manufacturer,
                              superior.model_year,
                              citizen.job, superior.rating)
            for subordinate in citizen.get_direct_subordinates():
                citizen.remove_subordinate(subordinate.cid)
                new_sup.add_subordinate(subordinate)
            new_citizen.add_subordinate(new_sup)
            for subordinate in superior.get_direct_subordinates():
                superior.remove_subordinate(subordinate.cid)
                new_citizen.add_subordinate(subordinate)
            new_citizen.remove_subordinate(citizen.cid)
            if not sup_superior:
                self._head = new_citizen
            else:
                sup_superior.add_subordinate(new_citizen)
                sup_superior.remove_subordinate(superior.cid)
            return new_citizen
        else:
            superior = citizen.get_superior()
            sup_superior = superior.get_superior()
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        if citizen == self._head:
            return
        if citizen.rating < citizen.get_superior().rating:
            return
        if isinstance(citizen.get_superior(), DistrictLeader):
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        superior = citizen.get_superior()
        if superior is None:
            subordinate = citizen.get_direct_subordinates()
            if not subordinate:
                self._head = None
            else:
                head = citizen.get_highest_rated_subordinate()
                head.set_superior(None)
                subordinate.remove(head)
                self._head = head
                for i in subordinate:
                    head.add_subordinate(i)
        else:
            for citizens in citizen.get_direct_subordinates():
                superior.add_subordinate(citizens)
            superior.remove_subordinate(cid)
